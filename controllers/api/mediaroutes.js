...
router.get('/songs/:title', withAuth, async (req, res) => {
  ...
  spotifyApi.searchTracks(req.params.title).then(
    function (data) {
      ...
      return 
      console.log(
        'Here is the track name of the first search result: ',
        JSON.stringify(data.body.tracks.items[0].name)
      );
      console.log(
        'Here is the URL for the first search result: ',
        JSON.stringify(data.body.tracks.items[0].external_urls.spotify)
      );
      console.log(
        'Here is the Spotify ID for the first search result: ',
        JSON.stringify(data.body.tracks.items[0].id)
      );
      console.log(
        'Here is the first artist listed for the first search result: ',
        JSON.stringify(data.body.tracks.items[0].artists[0].name)
      );
      console.log(
        'Here is the album name for the first search result: ',
        JSON.stringify(data.body.tracks.items[0].album.name),
        '\n\n---------------------\n'
      );
      ...
      artistResults = data.body.tracks.items.map((ele) =>
        ele.artists.map((ele) => ele.name)
      );
